...
export default class PartyMap extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      filter: 'perDistrict', checked: [true, false, false],
  ...
  componentWillMount() {
    ...
    let qString = `${config.apiUrl}/api/shape/zim_assembly_house2013_district`;
    console.log(qString);
    axios({
      method: 'get',
      url: qString,
      ...
      .then(response => {
        ...
        this.setState({ shapeDistrict: JSON.parse(response.data.data),shapeIsLoaded: true });
      })
      .catch(function (error) {
        console.log(error);
      });
    let qString2 = `${config.apiUrl}/api/shape/zim_assembly_house2013_province`;
    axios({
      method: 'get',
      url: qString2,
      headers: {
    ...
    if (this.state.filter == 'perDistrict') {
      this.setState({ grades: this.props.gradesProvince });
    }
  }
  ...
  style(feature) {
    ...
    return {
      fillColor: this.getColorRegElg(PROPERTY, ["#b5d7eb", "#90c1e1", "#6aacd8", "#3e96cf"], this.state.grades),
      weight: 1.2,
      opacity: 0.9,
      color: 'white',
      dashArray: '1',
      ...
      <div className="topMap">
        {this.state.shapeIsLoaded ? <Map maxZoom={8} center={[-18.9, 28]} keyboard={false} scrollWheelZoom={false} zoom={7} minZoom={5} style={{ height: "100vh", width: "100%", position: "relative" }}>
          ...
          <Control position="bottomright" >
            <MapKey grades={this.state.grades} percentageSign={this.state.percentageSign} keyTitle={this.state.keyTitle} colorSet={["#b5d7eb", "#90c1e1", "#6aacd8", "#3e96cf"]} key={this.state.filter} />
          </Control>
            ...
            <div className="col-lg-12 col-sm-2 col-sm-offset-2 col-lg-offset-1">
              <div className="well MenuShadow SideMenuePosition info-card-font">
                ...
                <section className='row col-md-12' >
                  ...
                  <div className="md-radio md-radio-inline" style={{ margin: '10px 0' }}>
                    <input id="4" type="radio" name="g3" value={1} onClick={this.handleRadioFilter.bind(this, 'perProvince')} checked={this.state.checked[1]} />
