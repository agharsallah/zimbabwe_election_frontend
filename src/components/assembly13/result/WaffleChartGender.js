...
export default class WaffleChartGender extends Component {
    constructor(props) {
        super(props);
        this.state = {
            shapeIsLoaded: false
    ...
    render() {
        ...
        return (
            <div className="content-inner" key={this.props.SelecteProvince + this.props.subFilterGender}>
            <h4 className="subheaderHover">Hover on the boxes for more info</h4>
                {
                    /* this.props.subFilterGender=!'All'? */
                    (this.props.subFilterGender == 'male' ?
                        this.props.assembly_house_res13.map(function (constituencyElm) {
                            constituencyElm.winner_m == '1' ? colorBox = '#22A8DB' : colorBox = 'white';
                            return (
                        ...
                        :
                        this.props.subFilterGender == 'female' ?
                            this.props.assembly_house_res13.map(function (constituencyElm) {
                                constituencyElm.winner_m == '0' ? colorBox = '#FC0F3A' : colorBox = 'white';
                                return (
                                    <a key={constituencyElm.winner_name}
                            ...
                            this.props.all_assembly_house_res13.map(function (constituencyElm) {
                                    constituencyElm.winner_m == '1' ? colorBox = '#22A8DB' :   colorBox = '#FC0F3A';
                                    return (
                                        <a key={constituencyElm.winner_name}
                                            ...
                                            <div className="tooltiptext">
                                                ...
                                                <h5 style={{ color: '#FE9187' }}>{VOTES_PERCENTAGE}: <span style={{ color: '#fff' }} >  {(parseInt((constituencyElm.winner_votes).replace(/,/g, '')) * 100 / parseInt((constituencyElm.total_votes).replace(/,/g, ''))).toFixed(2) + ' %'} </span> </h5>
                                                <h5 style={{ color: '#FE9187' }}>{VOTES_NUMBER}: <span style={{ color: '#fff' }} >  {commaNum((constituencyElm.winner_votes).replace(/,/g, '')) + ' votes'} </span> </h5>
                                                <h5 style={{ color: '#FE9187' }}>Party: <span style={{ color: '#fff' }} >  {constituencyElm.party_winner} </span> </h5>
